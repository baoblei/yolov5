...
class DetecResultQ:
    def __init__(self, m=0, results=[]):
        self.header = 0xff 
        self.type = 0xaa    
        self.m = m   # 识别数量
        self.results = results # 识别结果，最多五个目标
        self.check = 0x00  # TODO: 校验
...
def run(
    ...
    while True:
        ...
        # 是： 
            # 获取rtsp地址
            # recv = 'rtsp://192.168.8.88:554/av0_0'
            # source = str(recv)
            # dataloader()
            if webcam:
            ...
            for path, im, im0s, vid_cap, s in dataset:
                ...
                # Process predictions
                # 返回结果, TODO: 初始化？
                resultQ = DetecResultQ(m=0,results=[])  
                    ...
                    if len(det):
                        ...
                        for *xyxy, conf, cls in reversed(det):   # box confidence class
                            ...
                            if save_img or save_crop or view_img:  # Add bbox to image
                                ...
                                # TODO: 在im0这一帧的tcp发送结构体中添加对应的检测结果
                                result = DetectResult(cls, xyxy[0], xyxy[1], xyxy[2]-xyxy[0], xyxy[3]-xyxy[1], confidence)
                                resultQ.results.append(result)
                                resultQ.m = resultQ.m + 1
                            if save_crop:
                                save_one_box(xyxy, imc, file=save_dir / 'crops' / names[c] / f'{p.stem}.jpg', BGR=True)

                    # Stream results
                    im0 = annotator.result()
                    # TODO:发送send()im0帧的结果resultQ
                    # send(resultQ)
                    if view_img:
...
def parse_opt():
    ...
    parser.add_argument('--classes',default=[0,1], nargs='+', type=int, help='filter by class: --classes 0, or --classes 0 2 3')
